...
|---|---|---|---|
| Pool | O(1) | O(1) | Fixed-size objects (e.g. queue nodes) |
| Arena | O(1) | O(1) reset-only | Short-lived scratch memory, ring buffer arrays |
| Slab | O(n pools) | O(n pools) | Variable-size objects with known size classes |
...
Maintains a free-list inside a caller-supplied buffer. Every block is the same
size. Allocation and deallocation are both O(1).

```c
/* Backing storage — sized for N blocks of T. */
static unsigned char buf[sizeof(T) * N];

mem_pool_t pool;
pool_init(&pool, buf, sizeof(T), N);

T *obj = pool_alloc(&pool);   /* returns NULL if exhausted */
/* ... use obj ... */
pool_free(&pool, obj);
```

**Fields** (informational — treat as opaque):

| Field | Meaning |
|---|---|
| `start_addr` | Start of the backing buffer |
...
Bump allocator. Allocations are sequential; individual frees are not supported.
Reset the entire arena with `arena_reset()`.
...
void *a = arena_alloc(&arena, 64);   /* returns NULL if out of space */
void *b = arena_alloc(&arena, 128);

arena_reset(&arena);                 /* frees everything at once */
```

**Typical use**: allocate the `t_node **buffer` array inside `ring_init()` once
at startup; the array is never individually freed.

---

## Slab Allocator

Wraps up to `SLAB_MAX_POOLS` (8) pools of increasing block size. `slab_alloc()`
picks the smallest pool that satisfies the requested size.

```c
static unsigned char s_buf[32];   /* 4 × 8-byte blocks  */
static unsigned char m_buf[128];  /* 8 × 16-byte blocks */
...
void *p = slab_alloc(&slab, 10);  /* uses the 16-byte pool */
slab_free(&slab, p);
```
